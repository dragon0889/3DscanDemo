...
void FaceTrackingRenderer2D::DrawDistances()
{
	...
	swprintf_s<sizeof(tempLine) / sizeof(WCHAR) > (tempLine, L"Eyes Center : %.0f mm", eyesCenterAvg*1000);
...
double FaceTrackingRenderer2D::CalculateDistance(PXCPoint3DF32 p1, PXCPoint3DF32 p2)
{
...
void FaceTrackingRenderer2D::DrawLandmark(PXCFaceData::Face* trackedFace)
{
	...
	double headWidth = CalculateDistance(m_landmarkPoints[53].world, m_landmarkPoints[69].world);
	double noseBridge = CalculateDistance(m_landmarkPoints[10].world, m_landmarkPoints[18].world);
	double eyesCenter = CalculateDistance(m_landmarkPoints[76].world, m_landmarkPoints[77].world);
	...
	currEyesCenterAvg = (currEyesCenterAvg*frameNum + eyesCenter) / (frameNum + 1);
...
void FaceTrackingRenderer2D::CalcDistances()
{
	...
	eyesCenterAvg = currEyesCenterAvg;
}